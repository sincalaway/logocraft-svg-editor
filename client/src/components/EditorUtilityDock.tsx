...
type HistorySnapshot = { labels: string[]; activeIndex: number };
...
export default function EditorUtilityDock() {
  ...
  return <>
    ...
    {historyOpen && <div className="history-float" role="dialog" aria-label="操作历史"><div className="flex items-center justify-between border-b border-white/10 px-3 py-2"><span className="section-label">历史记录</span><button className="text-xs text-[#aaa9ac] hover:text-white" onClick={() => setHistoryOpen(false)}>关闭</button></div><div className="max-h-64 overflow-y-auto">{history.labels.map((label, index) => <button key={`${label}-${index}`} onClick={() => { emit("logocraft-history-restore", index); setHistoryOpen(false); }} className={`history-item ${index === history.activeIndex ? "history-item-active" : ""}`}><span>{label}</span><span className="technical-number">{String(index + 1).padStart(2, "0")}</span></button>)}</div></div>}
