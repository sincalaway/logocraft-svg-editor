...
import { extractSvgColorPalette } from "@/lib/svg-colors";
...
type SvgLibraryItem = { id: string; name: string; dataUrl: string; markup: string; viewBox: string; createdAt: string; primaryColor?: string; colors?: string[] };
...
function prepareSvg(source: string, fallbackName: string): Omit<SvgLibraryItem, "id" | "createdAt"> {
  ...
  const palette = extractSvgColorPalette(root.innerHTML);
  return { name: fallbackName.replace(/\.svg$/i, "").trim() || "未命名 SVG", dataUrl: `data:image/svg+xml;charset=utf-8,${encodeURIComponent(root.outerHTML)}`, markup: root.innerHTML, viewBox, ...palette };
}
...
export default function SvgLibraryPanel() {
  ...
  const dispatchInsert = (asset: Omit<SvgLibraryItem, "id" | "createdAt">) => {
    const palette = asset.primaryColor ? { primaryColor: asset.primaryColor, colors: asset.colors ?? [] } : extractSvgColorPalette(asset.markup);
    window.dispatchEvent(new CustomEvent("logocraft-svg-library-insert", { detail: { ...asset, ...palette } }));
    setHoverPreview(null); setOpen(false); toast.success(`已插入「${asset.name}」。`);
  };
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="SVG 图库">
      <section className="modal-surface flex max-h-[min(760px,calc(100dvh-32px))] w-full max-w-5xl flex-col overflow-hidden border border-white/10 bg-[#1d1d20] shadow-2xl">
        <header className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div><p className="section-label">矢量资产档案</p><h2 className="mt-1 flex items-center gap-2 font-display text-[20px] font-bold text-[#fff8f0]"><LibraryBig size={18} className="text-[#ff6b35]" /> SVG 图库</h2></div><div className="flex items-center gap-2"><button className="action-button action-primary h-9 px-3 text-[10px]" onClick={() => fileRef.current?.click()}><Upload size={14} /> 上传 SVG</button><button className="tool-button h-9 w-9" title="关闭 SVG 图库" onClick={() => { setHoverPreview(null); setOpen(false); }}><X size={17} /></button></div></header>
    ...
    {open && hoverPreview && <aside className="svg-library-hover-preview" style={{ left: previewLeft, top: previewTop }}><div className="svg-library-hover-preview-art"><img src={hoverPreview.path} alt="" /></div><strong>{hoverPreview.name}</strong><span>{hoverPreview.categoryName ?? "SVG 资产"}</span></aside>}
  </>;
}
