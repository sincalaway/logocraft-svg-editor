...
分类索引采用两级延迟加载：应用只先请求 `svg-library/index.json`，用户选择分类后才请求对应的 `catalog/<分类>.json` 与缩略图，避免首次打开图库一次加载数千个 SVG。资产保留在 `client/public/svg-library/assets/`，会随 `dist/public` 直接进入 Cloudflare Pages 发布物。
...
Cloudflare Pages 免费计划的站点可包含最多 20,000 个文件，单个静态资产上限为 25 MiB；本次构建的 5,198 个文件及最大的 1.57 MB SVG 均处于范围内。[1]
